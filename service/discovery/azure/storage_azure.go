...
func handleStorageAccount(account storage.Account) voc.IsStorage {
	return &voc.ObjectStorageResource{StorageResource: voc.StorageResource{
		Resource: voc.Resource{
			ID:           quad.IRI(*account.ID),
			Name:         *account.Name,
			CreationTime: &account.CreationTime.Time,
		},
		...
		HttpEndpoint: &voc.HttpEndpoint{
			...
			TransportEncryption: voc.NewTransportEncryption(
				...
				*account.EnableHTTPSTrafficOnly,
				getIRIForTLSVersion(account.MinimumTLSVersion),
			),
		},
	}
}

func getIRIForTLSVersion(tlsVersion storage.MinimumTLSVersion) quad.IRI {
	switch tlsVersion {
	case storage.MinimumTLSVersionTLS10:
		return quad.IRI("cloud:TLS1.0")
	case storage.MinimumTLSVersionTLS11:
		return quad.IRI("cloud:TLS1.1")
	case storage.MinimumTLSVersionTLS12:
		return quad.IRI("cloud:TLS1.2")
