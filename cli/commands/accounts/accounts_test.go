...
func TestDiscoverAccount(t *testing.T) {
	...
	cmd := accounts.NewDiscoverAccountsCommand()
	err = cmd.RunE(nil, []string{"mock"})
	...
	cmd = accounts.NewListAccountsCommand()
	err = cmd.RunE(nil, []string{"mock"})

	// should be no error
	assert.Nil(t, err)

	// should contain our account then
	var response *orchestrator.ListAccountsResponse = &orchestrator.ListAccountsResponse{}
	...
	assert.Equal(t, "mock", account.AccountType)
