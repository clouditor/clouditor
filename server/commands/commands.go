...
func BindPersistentFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().String(config.DashboardCallbackURLFlag, config.DefaultDashboardCallbackURL, "The callback URL of the Clouditor Dashboard. If the embedded server is used, a public OAuth 2.0 client based on this URL will be added")
	...
	cmd.PersistentFlags().String(config.APIEmbeddedOAuth2ServerPublicURLFlag, config.DefaultEmbeddedOAuth2ServerPublicURL, "Specifies the OAuth 2.0 public URL")
	...
	_ = viper.BindPFlag(config.DashboardCallbackURLFlag, cmd.PersistentFlags().Lookup(config.DashboardCallbackURLFlag))
	...
	_ = viper.BindPFlag(config.APIEmbeddedOAuth2ServerPublicURLFlag, cmd.PersistentFlags().Lookup(config.APIEmbeddedOAuth2ServerPublicURLFlag))
